...
//  HttpClientDemo
//
//  Created by fanpyi on 17/7/14.
//  Copyright (c) 2014 www.beequick.cn. All rights reserved.
//
...
class HttpClient:NSObject,NSURLConnectionDataDelegate{
    ...
    func getDataDic(url:String) {
        downloadFromGetUrl(url, completionHandler: {(data: NSData?, error: NSError?) -> Void in
            if (error != nil){
    ...
    func downloadFromPostUrl(url:NSString,dic:NSDictionary, completionHandler:((data:NSData?,error:NSError?)->Void)){
        ...
        for key:AnyObject in dic.allKeys{
            var s = String(key as! String) + "=" + String(dic[key as! String] as! String);
            //var s=NSString(format:"\(key as String)=\(dic[key as String])")//
            param.addObject(s)
        }
